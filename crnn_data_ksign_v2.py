...
class InputGenerator(object):
    ...
    def __init__(self, gt_util, batch_size, alphabet, input_size=(255,32),
                grayscale=True, max_string_len=30, datagen = False):
    ...
    def generate(self, train=True):
        ...
        while True:
            while len(targets) < batch_size:
                ...
                ### For Korean (진우 추가)
                if '2560_1440' in img_name:
                    img_path = os.path.join(gt_util.image_path + 'HighQuality', img_name)
                elif '1280_720' in img_name:
                    img_path = os.path.join(gt_util.image_path + 'MidQuality', img_name)
                elif '640_360' in img_name:
                    img_path = os.path.join(gt_util.image_path + 'LowQuality', img_name)
                else:
