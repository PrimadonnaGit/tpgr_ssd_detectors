...
class PriorUtil(object):
    ...
    def plot_gt(self):
        ...
        for p in self.gt_polygons:
            ax.add_patch(plt.Polygon(p, fill=False, edgecolor='y', linewidth=2))
        # groud truth rboxes
        rboxes = self.gt_rboxes
        for rbox in rboxes:
            box = rbox_to_polygon(rbox)
            ax.add_patch(plt.Polygon(box, fill=False, edgecolor='b', linewidth=1))
    ...
    def plot_results(self, results=None, show_labels=False, color='r'):
        ...
        for r in results:
            ...
            ax.add_patch(plt.Polygon(xy_rec, fill=False, edgecolor=color, linewidth=1))
            if show_labels:
                label_name = '%.2f' % (r[5],)
